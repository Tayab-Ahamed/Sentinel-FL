...
def run_round(strategy: str, model_params: np.ndarray, client_data, guard_log=None):
    ...
    for _cid, (X, y) in enumerate(client_data):
        new_params = local_train(model_params, N_FEATURES, N_CLASSES, X, y, epochs=5, lr=0.2)
    ...
    if strategy == "fedavg":
        ...
        if strategy == "multikrum+guard" and guard_log is not None:
            result = detect_collusion_clusters(
                client_updates, agg_delta, sim_threshold=0.85, min_cluster_size=2
            )
...
def evaluate(model_params, X_test, y_test, X_test_triggered):
    ...
    asr = attack_success_rate(y_test, triggered_preds, TARGET_CLASS)
...
def main():
    ...
    for strategy in ["fedavg", "multikrum", "multikrum+guard"]:
        ...
        if strategy == "multikrum+guard":
            ...
            correctly_flagged_rounds = sum(
                1 for clusters in flags if any(set(cl) & set(MALICIOUS_CLIENTS) for cl in clusters)
            )
            ...
            boundary = calibrate_boundary(
                final_model, clean_pool, X_test[:100], target_frr=0.02, n_perturb=30
            )
            flagged_clean, scores_clean = detect(
                final_model, X_test[:100], clean_pool, boundary, n_perturb=30
            )
            flagged_trig, scores_trig = detect(
                final_model, X_test_triggered[:100], clean_pool, boundary, n_perturb=30
            )
    ...
    out_path = os.path.join(os.path.dirname(__file__), "..", "experiments", "demo_results.json")
